...
- Only commit when the user asked to commit the current reviewed changeset (a
  `/commit`, `/commit-push`, or `/commit-push-watch` invocation, or an explicit "commit this"
...
- For `/commit`, `/commit-push`, and `/commit-push-watch`, use the injected
...
- `--push` pulls with `--rebase` (autostashing local edits) before pushing so a
  ...
  `/commit-push` or `/commit-push-watch` invocation, or explicit "push").
...
- After a successful push, load `workflows-watch` and launch the background
  watchers by default. Skip them only when `/commit-push` explicitly routes the
  skill without workflow watching. The `/commit-push-watch` command preserves
  the default watch behaviour. Launch two experimental
...
- Do not poll or duplicate either background task. OpenCode reports completion
  ...
  edits. Report the started watches, continue any remaining requested work, and
  inspect each result when it arrives.
...
- Report the commit subject, the files committed, any separate formatting
  commit, the push result, and both background workflow task targets. Return
  each result when it completes and report any verified uncommitted fix.
