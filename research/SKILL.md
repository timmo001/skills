...
3. **Fan out once when it helps.** When running interactively, broad or parallel reading may be delegated to terminal read-only subagents such as `explore`, `general-readonly`, or `researcher-readonly`. When running as a delegated subagent, do not use `task`; complete the assigned reading yourself and return it to the parent. Source ranking, claim verification, and synthesis stay with the interactive researcher.
