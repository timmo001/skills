...
For induction of an existing GitHub repository, resolve its actual host and owner/name from the selected remote, check **Choose and create the licence**, and complete **Watch the repository** after induction succeeds. Induction alone does not request the creation, settings, CI or ruleset steps below.

## Workflow

1. **Resolve the destination and visibility before creation.** Establish the host, owner, repository name and local source. Use an explicitly requested public/private choice, or infer it only from unambiguous task context such as a stated private project. An account's other repositories, a local directory, or GitHub's defaults do not establish visibility. If visibility is unspecified and cannot be inferred, ask public or private with the question tool before creating anything; fall back to chat when unavailable. Record the chosen visibility and its basis. Complete **Choose and create the licence** before creation or the first authorised push.
...
6. **Report the result.** Include repository URL and visibility, licence choice and source, verified settings and watching status with the authenticated account, workflows added or retained with source refs, first-push/CI evidence, ruleset result, and any declined or deferred setup. If no push was authorised, clearly identify the post-push steps still pending.

## Choose And Create The Licence

- Inspect existing `LICENSE*`, `COPYING*` and package licence metadata first. Preserve an existing licence and its notices; do not replace it with the default. Ask about conflicting declarations before changing them.
- When no licence exists, ask which licence to use with the question tool (chat fallback). For public repositories, offer **Apache 2.0 (Recommended)** first, another licence, or no licence. Apache 2.0 is the proposed default, not permission to apply it without an answer. For private repositories, ask whether to add a licence without assuming the public default. An explicit licence choice in the current request already answers the question.
- Source the selected text from GitHub's licence templates, available through the Licenses API. Discover keys with `gh repo license list` or `GET /licenses`; Apache 2.0 uses `apache-2.0`. Retrieve the template body, for example:

  ```bash
  gh api --hostname "$HOST" licenses/apache-2.0 --jq .body
  ```

  Use the selected key for other licences. Fetch successfully before writing the root `LICENSE` file. Do not bundle internal template copies, reconstruct legal text from memory, or shorten it.
- A licence from a known existing repository is also an acceptable source. Inspect its full text, verify the chosen licence and record the source path and commit. Account for project-specific notices rather than blindly copying another project's owner or year. If the source cannot be verified, report that and leave creation pending.
- Fill project-specific template fields from the actual copyright holder and year, asking when unclear. Preserve legal terms and instructional examples. For an existing local source, add `LICENSE` locally and keep package licence metadata consistent with the selected SPDX identifier. Leave committing and pushing to explicit authorisation. For remote-only creation, GitHub can generate the file with `gh repo create --license <key>` when its initial commit is authorised; do not initialise a separate remote history for an existing local repository.
- Verify the final file against the retrieved source, allowing only the intended project-specific substitutions, and report the licence and source URL or repository ref. If the user chose no licence, record that choice without creating a file.
...
- [GitHub licence templates API](https://docs.github.com/en/rest/licenses/licenses)
