...
| [`session-coordination`](./session-coordination/) | Coordinate requested multi-agent work through visible Herdr sessions, bounded assignments, and owned-session cleanup. Use only when the user requests multi-agent coordination or selects a coordinator agent. |
