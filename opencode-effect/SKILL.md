...
description: Develop and migrate OpenCode V2 plugins, clients, SDK hosts, and HTTP API integrations. Use for the OpenCode plugin API, `@opencode/client`, `@opencode/sdk`, server API, Effect entrypoints, or V1-to-V2 API migration.
---

# OpenCode V2 Development

Use OpenCode V2 and its Effect-native APIs unless the user explicitly asks for
a Promise-only integration or hybrid compatibility.
...
- Write plugins with `@opencode/plugin/effect`, `Plugin.define`, and an
  `effect` function. Use the plugin `Scope` for registrations, finalizers, and
  scoped fibers instead of manual Promise cleanup.
- Connect applications with `@opencode/client/effect`. Compose operations
  with `Effect`, consume subscriptions as `Stream` values, and use
  `@opencode/client/effect/service` for the Node background service.
- Embed OpenCode with `@opencode/sdk/effect`. Own the host with
...
- Use `@opencode/client`, `@opencode/client/service`, or the Promise SDK
