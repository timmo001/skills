...
`textAlign` pads each rendered line by display-cell width, not string length. After wrapping, each rendered line gets its own pad.

A 10-column `textAlign: "center"` view of `hi` then `world` draws `hi` with four cells of pad on each side. `world` gets two cells on the left and three on the right.

See [Text](/docs/components/text) for the component option.
