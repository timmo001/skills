...
2. **Implementation:** Give numbered, dependency-ordered code changes. For each step:
   ...
   - Apply `testing` before planning test edits. Name the meaningful failure or important behaviour each added test protects; an existing test file does not itself justify more coverage. Keep test edits beside the behaviour they verify, with their concrete benefit.
...
- Safeguards, compatibility layers, abstractions, and migrations address an observed contract or regression risk rather than speculative hardening. Test edits satisfy `testing`; a generic regression risk is not enough.
