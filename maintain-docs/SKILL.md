...
3. **Investigate the changes.** For each changed area with a likely doc gap or accuracy risk, read the change and the relevant existing docs in detail before writing. Delegate independent areas to parallel read-only investigations where the environment supports it: internal code-reading for the changed subsystems, and external primary-source verification (library, API, or spec behaviour) when a doc claim depends on it. Synthesise the findings yourself.
...
5. **Preview, then write.** List the intended documentation updates (file, section, and why), including corrections to inaccurate existing claims, before editing. Then write in-code and in-repo docs directly. For external locations that passed the gate, write in place; otherwise output the proposed changes for the user to apply.
